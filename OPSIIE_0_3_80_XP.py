...
def boot_up_sequence():
    ...
    print(pastel_cyan(f"\n[System] Booting OPSIIE v.0.3.80 for {current_user}."))
    time.sleep(2)
...
def display_status():
    ...
    try:
        ...
        print(pastel_yellow(f"System reboot in progress, do not turn off the machine {call_name or 'operator'}"))
...
def get_hf_image_headers():
    """Bearer token for Hugging Face Inference API; never hardcode secrets in source."""
...
def generate_image_from_prompt(prompt):
    ...
    try:
        hf_headers = get_hf_image_headers()
        if not hf_headers:
            error_message = (
                "Error: Set HUGGINGFACE_API_KEY or HF_TOKEN in your .env file for /imagine (Hugging Face Inference)."
            )
...
def handle_imagine_command(command):
    ...
    if "model" in command:
        ...
        try:
            hf_headers = get_hf_image_headers()
            if not hf_headers:
                print(pastel_red("Error: Set HUGGINGFACE_API_KEY or HF_TOKEN in .env to validate or use image models."))
                return
            new_api_url = f"https://api-inference.huggingface.co/models/{new_model_name}"
            test_response = requests.get(new_api_url, headers=hf_headers)
...
while True:
    ...
    if command in ['ask', 'markets', 'dna', 'send', '0x'] and not is_master_user():
        handle_restricted_command()
        print()
        continue
