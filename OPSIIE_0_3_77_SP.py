...
def display_splash():
    ...
    try:
        ...
        print(Fore.RED + f"Error fetching opsiieboot.mp3: {str(e)}")
...
def set_mode(mode):
    global current_mode
    current_mode = mode
    print(Fore.LIGHTCYAN_EX + f"Mode set to: {mode}")
...
def handle_restricted_command():
    ...
    message = "This command requires Master User (R-Grade) privileges. Please contact your system administrator for access."
...
try:
    ...
    print(Fore.RED + f"Error initializing MusicGen model: {str(e)}")
    musicgen_model = None
    musicgen_processor = None
    ...
    for attempt in range(retries):
        try:
            ...
            if response.status_code == 200:
                ...
                print(Fore.YELLOW + f"[Warning] Eleven Labs API check attempt {attempt + 1} failed: Status code {response.status_code}")
        except Exception as e:
            print(Fore.YELLOW + f"[Warning] Eleven Labs API check attempt {attempt + 1} failed: {str(e)}")
        time.sleep(2)
    return False
...
def speak_response(text):
    ...
    if not text:
        print(Fore.RED + "Error: Cannot speak an empty or None response.")
        return
    ...
    if response.status_code == 200:
        ...
        try:
            ...
            print(Fore.RED + f"Error deleting the temporary audio file: {str(e)}")
    
    else:
        print(Fore.RED + f"Error with Eleven Labs API: {response.status_code}")
...
def load_custom_sounds(custom_words):
    ...
    for word, path in custom_words.items():
        ...
        try:
            ...
            print(Fore.RED + f"Error loading {word} from {path}: {e}")
    return loaded_sounds
...
def toggle_voice_mode(mode):
    ...
    if mode == "/voice":
        ...
        print(Fore.LIGHTGREEN_EX + "Verbal communications protocol initialized.")
        ...
        while voice_mode_active:
            ...
            if voice_text:
                ...
                print(Fore.YELLOW + "No voice input detected. Exiting voice mode.")
                voice_mode_active = False
                agent_voice_active = False
                break
        ...
        agent_voice_active = True
        print(Fore.LIGHTGREEN_EX + "SCI voice responses activated. You can keep typing your input.")
        speak_response("SCI voice responses activated.")

    elif mode == "/voice2":
        voice_mode_active = True
        agent_voice_active = False
        print(Fore.LIGHTGREEN_EX + "User mic input enabled. Agent responds in text only.")
        ...
        voice_mode_active = False
        agent_voice_active = False
        print(Fore.RED + "Voice mode deactivated.")
        speak_response("Verbal communication protocol bridge collapsed successfully.")
    ...
    with mic as source:
        ...
        while voice_mode_active:
            try:
                ...
                print(Fore.LIGHTWHITE_EX + f"USER (via voice): {voice_text}")

                # Step 3: Check listening timeout
                if time.time() - start_time > max_listening_time:
                    print(Fore.YELLOW + "Max listening time reached. Processing captured input.")
                    break

                return accumulated_speech.strip()

            except sr.WaitTimeoutError:
                print(Fore.YELLOW + "No speech detected within the timeout. Listening stopped.")
                break
                ...
                print(Fore.RED + f"Error during voice recognition: {str(e)}")
...
def voice_to_text(audio):
    ...
    try:
        ...
        print(Fore.RED + f"Error in voice recognition: {str(e)}")
        return ""
...
def listen_for_voice_command(timeout=5):
    ...
    while time.time() - start_time < timeout:
        with mic as source:
            print(Fore.LIGHTCYAN_EX + f"Listening for voice commands... You have {timeout} seconds to say 'voice' to continue in voice mode.")
            ...
            try:
                ...
                print(Fore.LIGHTYELLOW_EX + "Waiting for a clear command...")
            except sr.WaitTimeoutError:
                print(Fore.YELLOW + "Timeout reached, no voice command detected.")
                break

    # If "voice" is detected, initialize voice mode
    if "voice" in voice_text:
        print(Fore.LIGHTCYAN_EX + "Lazy mode triggered!")
        toggle_voice_mode("/voice")
    else:
        # If no voice command is detected, proceed with text interface
        print(Fore.YELLOW + "Loading text interface")
...
# |___/ |_| |_|_\___/_/ \_\_|  |_| |_|_\___|___/_|  \___/|_|\_|___/___|                                                                     
# *** Stream Response *** | 
...
def stream_response(prompt):
    ...
    print(Fore.LIGHTGREEN_EX + 'OPSIE:', end=' ')
...
def detect_emotion(frame):
    ...
    try:
        ...
        if dominant_emotion:
            ...
            print(Fore.RED + "[Error] No dominant emotion detected.")
            return None

    except Exception as e:
        print(Fore.RED + f"[Error] Failed to detect emotion: {str(e)}")
        return None
...
def facial_recognition_auth():
    ...
    for name, path in known_image_paths.items():
        try:
            ...
            if len(face_encodings) > 0:
                ...
                print(Fore.RED + f"[Error] No face found in the image for {name}.")
        except Exception as e:
            print(Fore.RED + f"[Error] Failed to load or process image for {name}: {str(e)}")
            sys.exit()

    if not known_encodings:
        print(Fore.RED + "[Error] Unauthorized actor detected. The UBA security protocol will now terminate operations.")
        sys.exit()

    video_capture = cv2.VideoCapture(0)

    if not video_capture.isOpened():
        print(Fore.RED + "[Error] Unable to access the camera. Make sure it's connected and working as expected.")
        sys.exit()
    ...
    while True:
        ...
        if not ret:
            print(Fore.RED + "[Error] Unable to capture video. Make sure your camera is properly connected and operational.")
        ...
        if dominant_emotion in ['angry', 'fear']:
            print(Fore.RED + f"[Security] Abnormal stress levels detected. Detected emotion: {dominant_emotion}.")
            print(Fore.YELLOW + "Please try again when your mental and emotional state is in normalized spectrum.")
            video_capture.release()
            sys.exit()

        # If no emotion is detected or it's a neutral/happy emotion, continue
        if dominant_emotion:
            print(Fore.GREEN + f"[Security] Emotional state: {dominant_emotion}. Proceeding with facial recognition.")
        else:
            print(Fore.YELLOW + "[Security] No strong emotion detected. Proceeding with facial recognition.")
        ...
        if len(face_encodings) == 0:
            if time.time() - start_time > 9 and not warning_printed:
                print(Fore.YELLOW + "[Warning] Step up to the camera. Make sure the lens is clear and not blocked.")
                warning_printed = True

            if time.time() - start_time > 18:
                print(Fore.RED + "[Error] Failed to authenticate: No face detected. Please ensure you are standing in front of the camera.")
                video_capture.release()
                sys.exit()
        else:
                ...
                for name, known_encoding in known_encodings.items():
                    ...
                    if match[0]:
                        ...
                        if user_info['arpa_id'].startswith('R'):
                            print(Fore.GREEN + f"[Security] Authentication successful. Master User recognized. Welcome, {current_user}!")
                            ...
                            print(Fore.GREEN + f"[Security] Authentication successful. Welcome, {current_user} ({call_name})!")

                        video_capture.release()
                        return current_user, call_name, db_params, dominant_emotion, public0x, arpa_id

            if not match_found:
                print(Fore.RED + "[Error] Unauthorized actor detected. The security protocol will now terminate operations.")
                video_capture.release()
                sys.exit()
...
def boot_up_sequence():
    ...
    if not user_name:
        print(Fore.RED + "[Security] Boot-up aborted due to failed authentication.")
        sys.exit()
    ...
    print(Fore.CYAN + f"\n[System] Booting OPSIIE v.0.3.75 for {current_user}.")
    time.sleep(2)
    ...
    try:
        conn = connect_db()
        if conn:
            print(Fore.GREEN + f"[Network] Secure connection to {db_params['dbname']} established.")
    except Exception as e:
        print(Fore.RED + f"[Error] Failed to connect to {db_params['dbname']}: {str(e)}")
        sys.exit()
    time.sleep(0.2)
    ...
    print(Fore.YELLOW + "[System] Verifying System Prompt Authenticity")
    try:
        if user_name:
            print(Fore.GREEN + f"[System] Genuine Inscribed ARPA OEM System Prompt, and user SoulSig Verified. Welcome, {user_name}.")
        ...
        print(Fore.RED + f"[Error] System Prompt Verification Failed: {str(e)}")
        sys.exit()
    time.sleep(0.8)
    ...
    if vector_db is None:
        print(Fore.RED + "Error: vector_db is None after initialization.")
        sys.exit()

    print(Fore.YELLOW + "[Hardware] Stress testing mnemonic matrix with simulations ...")
    try:
        test_conversations = fetch_conversations()
        if test_conversations:
            print(Fore.GREEN + "[Hardware] Mnemonic integrity: Nominal.")
        else:
            print(Fore.YELLOW + "[Hardware] No data retrieved from mnemonic_computer. Proceeding with empty mnemonic matrix.")
    except Exception as e:
        print(Fore.RED + f"[Error] Mnemonic matrix test failed: {str(e)}")
        sys.exit()
    time.sleep(2)

    # Step 5: Response Engine Initialization
    print(Fore.YELLOW + "[SCI Systems] Initializing OPSIE response engine ...")
    try:
        test_prompt = "Hello, Opsie. How are you?"
        response = ollama.chat(model='llama3', messages=[{'role': 'user', 'content': test_prompt}])
        print(Fore.GREEN + "[SCI Systems] Response engine confidence: Trustworthy.")
    except Exception as e:
        print(Fore.RED + f"[Error] Opsie2 response engine initialization failed: {str(e)}")
        sys.exit()
    time.sleep(0.7)

    # Step 6: Verify Sensory Modules and APIs
    print(Fore.YELLOW + "[SCI Systems] Verifying sensory modules, APIs, and other tools ...")
    try:
        ...
        print(Fore.GREEN + "[SCI Systems] Sensor Status: Verified and functioning as expected.")
    except Exception as e:
        print(Fore.RED + f"[Error] Sensory modules or API checks failed: {str(e)}")
        sys.exit()
    time.sleep(0.5)

    # Step 7: Web3 Initialization
    print(Fore.YELLOW + "[Network] Initializing Web3 Handler ...")
    try:
        web3 = Web3(Web3.HTTPProvider('https://mainnet.base.org'))
        if web3.is_connected():
            web3_handler = Web3Handler(known_user_names)
    except Exception as e:
        print(Fore.RED + f"[Error] Web3 initialization failed: {str(e)}")
        web3_handler = None
    time.sleep(0.3)

    # Step 8: Mail Systems Check
    print(Fore.YELLOW + "[Network] Verifying Mail Systems ...")
    try:
        if EMAIL and PASSWORD:
            print(Fore.GREEN + "[Network] Mail Systems: Connected")
    except Exception as e:
        print(Fore.RED + f"[Error] Mail systems verification failed: {str(e)}")
    time.sleep(0.5)

    # Step 9: Dream Engine Initialization
    print(Fore.YELLOW + "[SCI Systems] Initializing Dream Engine ...")
    try:
        if torch.cuda.is_available() or device == 'cpu':
            print(Fore.GREEN + "[SCI Systems] Dream Engine Modules: Image, Music, Video Generation Ready")
    except Exception as e:
        print(Fore.RED + f"[Error] Dream Engine initialization failed: {str(e)}")
    time.sleep(0.5)

    # Step 10: Advanced Analysis Systems
    print(Fore.YELLOW + "[SCI Systems] Initializing Advanced Analysis Systems ...")
    try:
        # DNA Analysis check
        test_sequence = generate_random_dna(10)
        if is_dna(test_sequence):
            print(Fore.GREEN + "[SCI Systems] DNA Analysis Module: Operational")
        
        # Market Intelligence check
        test_ticker = "SPY"
        yf.Ticker(test_ticker).info
        print(Fore.GREEN + "[SCI Systems] Market Intelligence Feed: Active")
        
        # File Reader check
        print(Fore.GREEN + "[SCI Systems] TAF-3000 File Reader: Online")
    except Exception as e:
        print(Fore.RED + f"[Error] Advanced Analysis Systems initialization failed: {str(e)}")
    time.sleep(0.5)

    # Step 11: Agentic Network Initialization
    print(Fore.YELLOW + "[Network] Initializing ARPA Nexus ...")
    try:
        ...
        if agent_status:
            print(Fore.GREEN + f"[Network] Active Agents: {', '.join(agent_status)}")
        else:
            print(Fore.YELLOW + "[Network] Warning: No agents currently active")

        # Initialize ChromaDB client for vector operations
        client = chromadb.Client()
        print(Fore.GREEN + "[Network] ARPA Nexus ENA: Ready")

        # Initialize model APIs
        for model_name, model_info in MODEL_APIS.items():
            if 'api_url' in model_info:
                print(Fore.GREEN + f"[Network] {model_info['display_name']} API endpoint configured")

    except ValueError as ve:
        print(Fore.RED + f"[Error] ARPA Nexus configuration error: {str(ve)}")
    except Exception as e:
        print(Fore.RED + f"[Error] ARPA Nexus initialization failed: {str(e)}")
    time.sleep(0.5)

    # Final System Status
    print(Fore.LIGHTYELLOW_EX + "[System] All systems are nominal.")
    time.sleep(1.5)
    ...
    try:
        ...
        print(Fore.RED + f"Error fetching gb.mp3: {str(e)}")
...
def display_status():
    ...
    global voice_mode_active, agent_voice_active
    try:
        print(Fore.YELLOW + "Checking system status...")
        speak_response("Entering self-diagnosis mode...")
        print(Fore.YELLOW + "System reboot in progress, do not turn off the machine {call_name}")
        speak_response("Holistic system reboot in progress. Please, standby.")
        time.sleep(3) 
        boot_up_sequence()
    except Exception as e:
        print(Fore.LIGHTRED_EX + f"Error during status update: {str(e)}")
...
def connect_db():
    ...
    if not db_params:
        print(Fore.RED + "Error: Database parameters not set. Authentication may have failed.")
        sys.exit()
...
def ensure_vector_db_exists():
    ...
    try:
        ...
        print(Fore.GREEN + f"Collection '{vector_db_name}' found and ready for use.")
        ...
        if vector_db.count() == 0:
            ...
            if conversations:
                ...
                print(Fore.GREEN + f"[Hardware] Vector DB Collection '{vector_db_name}' created and populated with mnemonic specimens.")
        return vector_db
    except ValueError:
        print(Fore.YELLOW + f"[Hardware] Temporal active Chroma Vector DB Collection '{vector_db_name}' does not exist. Creating it now...")
...
def store_conversations(prompt, response):
    ...
    if vector_db is None:
        vector_db = ensure_vector_db_exists()
        if vector_db is None:
            print(Fore.RED + "Error: vector_db is None after attempting to initialize in store_conversations.")
            return
    ...
    try:
        ...
        print(Fore.RED + f"Error adding to vector_db: {str(e)}")
        ...
        try:
            ...
            print(Fore.RED + f"Error adding to vector_db after re-initialization: {str(e)}")
...
def remove_last_conversation():
    ...
    with connect_db() as conn:
        with conn.cursor() as cursor:
            ...
            if last_id:
                ...
                print(Fore.YELLOW + "No conversations found to delete.")
                return
    ...
    if vector_db is None:
        vector_db = ensure_vector_db_exists()
        if vector_db is None:
            print(Fore.RED + "Error: vector_db is None after attempting to initialize in remove_last_conversation.")
            return

    # Now remove from vector database using the global vector_db
    try:
        vector_db.delete(ids=[str(last_id)])
        print(Fore.GREEN + f"Conversation with ID {last_id} removed from vector_db.")
    except Exception as e:
        print(Fore.RED + f"Error deleting from vector_db: {str(e)}")
        # Optionally, re-initialize vector_db and retry
        vector_db = ensure_vector_db_exists()
        try:
            vector_db.delete(ids=[str(last_id)])
            print(Fore.GREEN + f"Conversation with ID {last_id} removed from vector_db after re-initialization.")
        except Exception as e:
            print(Fore.RED + f"Error deleting from vector_db after re-initialization: {str(e)}")
...
def create_vector_db(conversations, vector_db=None):
    ...
    if conversations:
        for c in tqdm(conversations, desc='[Hardware] Populating vector database'):
        ...
        print(Fore.YELLOW + f"No conversations found to populate the vector database. An empty collection '{vector_db_name}' was created.")
    return vector_db
...
def retrieve_embeddings(queries, results_per_query=10):  # Increase the results per query
    ...
    vector_db = ensure_vector_db_exists()
    if not vector_db:
        print(Fore.RED + "Error: Unable to query vector database as the collection does not exist.")
        return set()
...
def recall(prompt):
    ...
    if embeddings:
        ...
        print(Fore.RED + "No relevant memories retrieved.")
...
def generate_image_from_prompt(prompt):
    ...
    try:
        ...
        try:
            ...
            print(Fore.LIGHTYELLOW_EX + f"Image generated and saved as '{image_filename}'.")
...
def handle_imagine_command(command):
    ...
    if "model" in command:
        ...
        if not new_model_name:
            ...
            print(Fore.LIGHTBLUE_EX + current_model_message)
            if agent_voice_active or voice_mode_active:
        ...
        try:
            ...
            if test_response.status_code == 200:
                ...
                print(Fore.LIGHTGREEN_EX + success_message)
                if agent_voice_active or voice_mode_active:
...
def handle_weblimit_command(command):
    ...
    try:
        ...
        if len(args) == 1:  # No limit value was provided
            print(Fore.YELLOW + f'Current webpage character extraction limit: {webpage_char_limit}')
            return
        ...
        if new_limit.lower() == 'default' or new_limit == '1000':  # Set to default if specified
            webpage_char_limit = 1000
            print(Fore.YELLOW + 'Webpage character extraction limit set to default (1000 characters).')
            return
        ...
        if 500 <= new_limit <= 5000:
            ...
            print(Fore.YELLOW + f'Webpage character extraction limit successfully set to {new_limit}.')
        else:
            print(Fore.LIGHTRED_EX + 'Error: Character extraction limit must be between 500 and 5000. Please try again.')

    except (ValueError, IndexError):
        print(Fore.LIGHTRED_EX + 'Error: Invalid input. Please enter a valid number between 500 and 5000.')
...
def handle_file_reading(file_path, follow_up_prompt=None):
    ...
    # Load XLSX file
        ...
        print(Fore.RED + f"Unsupported file type: {file_path}")
        return
    ...
    if file_content is not None:
        ...
        if follow_up_prompt:
            ...
            print(Fore.LIGHTMAGENTA_EX + "File Manager TAF-3000: " + response)
        else:
            # Natural confirmation for file reading with agent-style response
            print(Fore.LIGHTYELLOW_EX + f"File Manager TAF-3000: The file '{file_path}' has been scanned successfully. You can now proceed with queries in regards to the file contents in a separate context window. To return to default conversational mode, type /close.")
            if voice_mode_active:
...
def process_follow_up_on_file(follow_up_prompt, file_content):
    ...
    if 'last_file' in file_context:
        ...
        print(Fore.YELLOW + "The file context window is now closed. You can proceed to default conversation with OPSIE or use /open to reopen the file.")
        if voice_mode_active:
...
def read_xlsx(file_path):
    ...
    try:
        ...
        print(Fore.RED + f"Error reading XLSX: {e}")
        return None
...
def read_txt(file_path):
    ...
    try:
        ...
        print(Fore.RED + f"Error reading TXT: {e}")
        return None
...
def handle_music_command(command):
    ...
    try:
        print(Fore.LIGHTCYAN_EX + "OPSIE is cooking bits & bytes... do not disturb.")

        # Generate a random expression for print
        print_expression = get_random_expression()
        print(Fore.LIGHTGREEN_EX + print_expression)
        ...
        print(Fore.LIGHTYELLOW_EX + f"Audible specimen generated and saved as '{output_file}'.")
...
def handle_user_query(prompt):
    ...
    if prompt.startswith('/room'):
        ...
        if match:
            ...
            if invalid_agents:
                print(Fore.RED + f"Invalid agent(s): {', '.join(invalid_agents)}")
                print(Fore.YELLOW + f"Available agents: {', '.join(AGENT_DISPLAY_NAMES[a] for a in valid_agents)}")
                return
            
            if not agents:
                print(Fore.RED + "No valid agents specified")
                print(Fore.YELLOW + f"Available agents: {', '.join(AGENT_DISPLAY_NAMES[a] for a in valid_agents)}")
                return
            
            current_room = Room(agents, room_prompt, get_opsie_response)
            print(Fore.LIGHTMAGENTA_EX + f"Room created with agents: {', '.join([AGENT_DISPLAY_NAMES[a.lower()] for a in agents])}")
            print(Fore.LIGHTYELLOW_EX + f"Room system prompt: {room_prompt}\n")
            ...
            try:
                ...
                print(Fore.LIGHTGREEN_EX + f"{AGENT_DISPLAY_NAMES['opsie']}: {opsie_intro}")
                if voice_mode_active:
                    speak_response(opsie_intro)
            except Exception as e:
                print(Fore.RED + f"Error in OPSIE introduction: {str(e)}")
            
            # Then each additional agent
            for agent in agents:
                try:
                    intro = get_agent_intro(agent, room_prompt, user_name)  # Pass user_name here
                    print(Fore.LIGHTBLUE_EX + f"{AGENT_DISPLAY_NAMES[agent.lower()]}: {intro}")
                    if voice_mode_active:
                        speak_agent_response(intro, agent)
                except Exception as e:
                    print(Fore.RED + f"Error in {agent} introduction: {str(e)}")
            
            return
        else:
            print(Fore.RED + "Invalid room format. Use: /room agent1: prompt  or  /room agent1, agent2: prompt")
            return
    ...
    # Handle room exit
        ...
        if current_room:
            current_room.close()
            current_room = None
            print(Fore.LIGHTYELLOW_EX + "Room closed. Returning to normal conversation mode.")
            if voice_mode_active:
                speak_response("Room closed. Returning to normal conversation mode.")
            return
        else:
            print(Fore.YELLOW + "No active room to close.")
            return
    ...
    if command == 'voiceoff':
        ...
        voice_mode_active = False
        agent_voice_active = False
        print(Fore.YELLOW + "Voice mode deactivated. Switching to text input mode.")
        return
    ...
    if prompt.lower().startswith('/soulsig'):
        ...
        if len(command_parts) == 1:
            ...
            print(Fore.LIGHTYELLOW_EX + "Your current Soul Signature is:\n")
            print(Fore.WHITE + soul_sig)  # Change the color of the content to white
            return
            ...
            if sub_command.lower() == 'wipe':
                ...
                print(Fore.LIGHTRED_EX + "Once your SoulSig is wiped, it ceases to live in the mnemonic matrix.\nPermanent data loss is imminent. Proceed with caution!\n")
                ...
                confirmation = input(Fore.LIGHTRED_EX + "Type 'confirm' if this is not a drill!: ").strip().lower()
                ...
                if confirmation == 'confirm':
                    ...
                    print(Fore.RED + "WARNING: SCI FATIGUE IMMINENT.\n")
                    print(Fore.YELLOW + "Your SCI agent might feel disoriented without your SoulSig. Consider inscribing a few new lines.\n")
                    print(Fore.YELLOW + "Visit /help soulsig for more info.\n")
                    save_known_user_names()  # Save changes to the file
                    
                else:
                    print(Fore.YELLOW + "Soul Signature wipe sequence terminated.")  # Inform the user that the wipe was cancelled
                return
                ...
                if temporary_soul_sig is not None:
                    ...
                    print(Fore.RED + "No temporary Soul Signature found to restore.\n")
                return
                ...
                print(Fore.LIGHTGREEN_EX + f"New inscription appended to Soul Signature: '{new_line}'")
                save_known_user_names()  # Save changes to the file
                return
    ...
    if command == '0x':
        ...
        print(Fore.YELLOW + "Note: Web3 commands now use the /0x prefix. For example: '/0x send base eth 0.1 to Ross'")
        return
        ...
        try:
            ...
            if prompt.lower().startswith('/ask g1 live'):
                if not G1_VOICE_LIVE:
                    print(Fore.RED + "Error: G1 voice agent ID not configured in .env file")
                    return
                
                try:
                    print(Fore.LIGHTGREEN_EX + "Initializing live conversation mode with G1 Black...")
                    start_live_g1_conversation()
                except Exception as e:
                    print(Fore.RED + f"Error in live conversation: {str(e)}")
                return
            ...
            if match:
                ...
                try:
                    ...
                    print(Fore.RED + f"Error querying {model_name}: {str(e)}")
            else:
                print(Fore.RED + "Invalid format for /ask command. Usage: /ask <model> <prompt> or /ask g1 live")
        except Exception as e:
            print(Fore.RED + f"Error processing ask command: {str(e)}")
        ...
        if not keyword:
            print(Fore.YELLOW + "Please provide a sector, company, or currency keyword. Usage: /markets <keyword>")
            return
        ...
        if len(args) == 1:
            ...
            print(Fore.RED + "Invalid usage of /help. Use /help or /help <command>")
    elif command == 'status':
        display_status()
    elif command == 'voice':
        ...
        if match:
            ...
            print(Fore.RED + "No valid file path found in the prompt.")
    elif command == 'open':
        if 'last_file' in file_context:
            print(Fore.YELLOW + "Reopened the last loaded file context. You can now query the file again.")
            if voice_mode_active:
                speak_response("Reopened the last loaded file context. You can now ask me anything about it.")
        else:
            print(Fore.RED + "No file has been loaded yet. Use /read <file> to load a file.")
            if voice_mode_active:
                speak_response("It seems like I've lost track of the file. Use /read to load a file.")
    elif command == 'close':
        file_context.clear()
        print(Fore.YELLOW + "File context window concluded. You can continue your convo with OPSIE.")
        if voice_mode_active:
...
while True:
    ...
    prompt = input(Fore.LIGHTWHITE_EX + 'USER:\n').strip()
    print()
    ...
    if prompt.lower().startswith('/weblimit'):
        try:
            ...
            print(Fore.LIGHTRED_EX + 'Error: No limit provided. Usage: /weblimit <number between 500 and 5000> or /weblimit default')
        print()
